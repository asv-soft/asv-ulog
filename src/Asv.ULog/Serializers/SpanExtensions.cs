...
namespace Asv.ULog
{
    public static class SpanExtensions
    {
            ...
            unsafe
            {
                ...
                try
                {
                    ...
                    fixed (char* charPointer = charBuffer)
                    {
                        ...
                        actualCharCount = encoding.GetDecoder().GetChars(bytePointer, span.Length, charPointer,
                            maxCharCount, flush: false);
        ...
        public static void CopyTo(this ReadOnlySpan<char> source, ref Span<byte> span, Encoding encoding)
        {
            unsafe
            {
                fixed (char* charPointer = source)
        ...
        public static unsafe void CopyTo(ref Span<byte> span, char* charPointer, int charCount, Encoding encoding)
        {
            fixed (byte* spanPointer = span)
            {
                
                var writtenBytes = encoding.GetBytes(charPointer, charCount, spanPointer, span.Length);
